...
class PauseStateTest extends ApplicationTest {
    ...
    void setUp() {
        ...
        state = new PauseState(stateManager);
    }
    ...
    @Test
    @DisplayName("resume() - View 색상 갱신")
    void testResumeRefreshesColors() {
        javafx.application.Platform.runLater(() -> {
            state.enter();
            state.createScene();
            
            // resume 호출 시 예외 발생하지 않음
            assertDoesNotThrow(() -> state.resume());
        });
        WaitForAsyncUtils.waitForFxEvents();
    }
    ...
    @Test
    @DisplayName("resume() - enter 전 호출")
    void testResumeBeforeEnter() {
        // resume이 enter 전에 호출되어도 예외 발생하지 않음
        assertDoesNotThrow(() -> state.resume());
    }
}