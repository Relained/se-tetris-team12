...
class PlayStateTest extends ApplicationTest {
    ...
    void testCreateSceneKeyHandler() {
        javafx.application.Platform.runLater(() -> {
            ...
            assertNotNull(scene.getOnKeyReleased());
        });
        WaitForAsyncUtils.waitForFxEvents();
    }
    
    @Test
    @DisplayName("enter() - 게임 로직, View, Controller 초기화")
    ...
    @Test
    @DisplayName("exit() - 게임 타이머 정지")
    void testExitStopsTimer() {
        state.enter();
        
        // exit를 여러 번 호출해도 예외 발생하지 않음
        assertDoesNotThrow(() -> {
            state.exit();
            state.exit();
        });
    ...
    @Test
    @DisplayName("resume() - 게임 타이머 재개")
    void testResumeStartsTimer() {
        state.enter();
        state.exit();
