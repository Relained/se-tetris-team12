...
class StartStateTest extends ApplicationTest {
    ...
    void setUp() {
        ...
        state = new StartState(stateManager);
    }
    ...
    void testEnterInitializesComponents() {
        ...
        // enter 후에 createScene이 정상 작동하는지 확인
        assertDoesNotThrow(() -> {
            javafx.application.Platform.runLater(() -> {
                Scene scene = state.createScene();
                assertNotNull(scene);
    ...
    void testCreateSceneWithoutEnter() {
        javafx.application.Platform.runLater(() -> {
            // enter 없이 createScene 호출하면 NullPointerException 발생할 수 있음
